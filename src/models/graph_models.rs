    ...
    Ty: EdgeType + Serialize + for<'de> Deserialize<'de>,
{
    pub fn new(name: &str) -> Self {
        ...
        fn ensure_runtime_edge_instance(&mut self, name: &str, edge_index: EdgeIndex) {
            if !self.space_ref.edges.iter().any(|r| r.index == edge_index) {
                self.space_ref.edges.push(Ref {
                    name: name.to_string(),
                    uuid: Uuid::new_v4(),
                    index: edge_index,
                });
        ...
        fn ensure_runtime_edge_kind(&mut self, name: &str, edge_kind_id: Uuid) {
            ...
            {
                self.space_ref.edge_kinds.push(EdgeKindRef {
                    name: name.to_string(),
                    uuid: edge_kind_id,
                });
            }
        ...
        fn clone_for_save(&self) -> Self {
            let mut cloned = Self {
                ...
                space_ref: self.space_ref.clone(),
                metadata: self.metadata.clone(),
                edge_kinds: self.edge_kinds.clone(),
                node_index_by_name: HashMap::new(),
            };
            cloned.space_ref.nodes.sort_by_key(|r| r.index.index());
            cloned.space_ref.edges.sort_by_key(|r| r.index.index());
            cloned
                .space_ref
                .edge_kinds
        ...
        pub fn top_k_nodes_by_energy(&self, k: usize) -> Vec<Ref<NodeIndex>> {
            let mut v: Vec<_> = self.space_ref.nodes.iter().cloned().collect();
